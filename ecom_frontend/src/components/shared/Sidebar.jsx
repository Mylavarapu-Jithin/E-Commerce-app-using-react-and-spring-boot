...
const Sidebar = ({ isProfileLayout = false }) => {
  ...
  const sideBarLayout = isAdmin ? adminNavigation : isSeller ? sellerNavigation : []; 
              ...
              {sideBarLayout.map(item => (
                <li key={item.name}>
                  <Link to={item.href} className={classNames(
                    pathName === item.href ? "bg-custom-blue text-white" : "text-gray-400 hover:bg-gray-800 hover:text-white",
