      ...
      <div className='flex flex-wrap'> 
        <div className='w-full lg:w-8/12 pr-4'> 
          <div className='space-y-4'>
            <div className='p-4 border rounded-lg shadow-sm space-y-1'>
                ...
                {cart?.map(item => (
                  <div key={item?.productId} className='flex items-center'>
                    <img src={`${import.meta.env.VITE_BACK_END_URL}/images${
                      item?.image
                    }`} alt='Product' className='w-12 h-12 rounded' /> 
                    <div className='text-gray-500'>
                      <p>{item?.productName}</p>  
                      <p>{item?.quantity} X ${item?.specialPrice} = ${
                        formatPriceCalculation(item.quantity, item.specialPrice)}</p> 
                    </div>
                  </div>    
                ))}      
              </div>
